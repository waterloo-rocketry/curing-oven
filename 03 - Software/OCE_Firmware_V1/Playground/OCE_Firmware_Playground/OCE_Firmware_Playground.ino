...
int Temp_Max_Part = 120 ; // Maximum part temperature - will throw a Program Not OK error if it is exceeded during the startup phase
...
void loop() {
  ...
  if(state==1 && !stateChange) // Enter the STARTUP State
  {
    ...
    while(ILOK && !RUN)
    {
      ...
      while(curs == 1 && SLT && !OUT)
      {
        ...
        if(SLT) // This seems to work well, except for thermistors 7 and 9
        {
          Serial.println("SLT") ;
  ...
  if(state == 2 && !stateChange) // Enter the CURE State
  {
    ...
    {
      error_I += error_P*ts ;  // issue - integral windup needs to be two cases - should be able to reduce the integral error
    }  
